...
##### jet inner angle #####
if jet_angle_inner:
    ax1.plot([-5*a*np.sin(jet_angle_inner),5*a*np.sin(jet_angle_inner)]+a_s,[-5*a*np.cos(jet_angle_inner),5*a*np.cos(jet_angle_inner)], color='white', ls=':', lw=1.5)
    ax1.plot([5*a*np.sin(jet_angle_inner),-5*a*np.sin(jet_angle_inner)]+a_s,[-5*a*np.cos(jet_angle_inner),5*a*np.cos(jet_angle_inner)], color='white', ls=':', lw=1.5)
...
ax1.arrow(-a_p, 0, a*np.sin(inclination), a*np.cos(inclination), color='k', head_width=0.03, head_length=0.06, zorder=40)
...
ax1.add_artist(primary)
# axes[0].add_artist(primary_roche)
ax1.scatter([a_s],[0], marker="*", s=400, color='darkorange', zorder=50)
ax1.scatter([0],[0], marker="x", s=50, color='red')



ax1.set_xlim(-1,1)
ax1.set_ylim(-0.501,0.501)
...
ax1.pcolormesh(X+a_s, Y, Z, vmin=-5, vmax = 1, cmap='Blues')

ax1.plot([a_s+3*np.tan(jet_tilt), a_s-3*np.tan(jet_tilt)], [-3, 3], color='k', ls='--')
...
primary = plt.Circle((0, 0), radius_p, color='darkorange', zorder=0.5)
height = 0.5
width  = 0.3
jet_tilt = pars_model['jet_tilt']
dimx = (-width,width)
...
y_length_min = width/np.tan(jet_angle_inner-jet_tilt)
y_length_max = width/np.tan(jet_angle_inner+jet_tilt)
x_length_min = height*np.tan(jet_angle_inner-jet_tilt)
x_length_max = height*np.tan(jet_angle_inner+jet_tilt)
##### jet inner angle #####
# if jet_angle_inner:
#     ax3.plot([-width*np.sin(jet_angle_inner-jet_tilt),width*np.sin(jet_angle_inner-jet_tilt)],[-width*np.cos(jet_angle_inner-jet_tilt),width*np.cos(jet_angle_inner-jet_tilt)], color='white', ls=':', lw=1.5)
#     ax3.plot([width*np.sin(jet_angle_inner+jet_tilt),-width*np.sin(jet_angle_inner+jet_tilt)],[-width*np.cos(jet_angle_inner+jet_tilt),width*np.cos(jet_angle_inner+jet_tilt)], color='white', ls=':', lw=1.5)
if jet_angle_inner:
    if y_length_min > height:

        ax3.plot([-x_length_min,x_length_min],[-height,height], color='white', ls=':', lw=1.5)
        ax3.plot([width,-width],[-y_length_max,y_length_max], color='white', ls=':', lw=1.5)

    elif y_length_max > height:

        ax3.plot([-width,width],[-y_length_min,y_length_min], color='white', ls=':', lw=1.5)
        ax3.plot([x_length_max,-x_length_max],[-height,height], color='white', ls=':', lw=1.5)
...
# ax3.add_artist(primary)
ax3.plot([height*np.tan(jet_tilt),-height*np.tan(jet_tilt)], [-height, height], color='k', ls='--')
ax3.scatter([0],[0], marker="*", s=440, color='black', zorder=40)
ax3.scatter([0],[0], marker="*", s=400, color='darkorange', zorder=50)

jetmesh = ax3.pcolormesh(X, Y, Z, vmin=-5, vmax = 1, cmap='Blues')

cb = fig.colorbar(jetmesh,ax=ax3, pad=0.01, aspect=40, label='Log scaled density')
cb.set_clim(-5, 1);
...
ax2.add_artist(primary_roche)
ax2.add_artist(secondary_roche)
ax2.scatter([a_s],[0], marker="*", s=400, color='darkorange', zorder=50)
ax2.scatter([0],[0], marker="x", s=50, color='red')

ax2.set_xlim(-1,1)
ax2.set_ylim(-0.501,0.501)
# axes[0].set_xlim(-1.5*a, 1.5*a)
# axes[0].set_ylim(-0.5*a,a)
...
print(Z)
##### BD: 3.962  IRAS: 3.99
# ax2.contour(X*a-a_p, Y*a, Z,[-2,3.962], cmap='RdGy')
ax2.contour(X*a-a_p, Y*a, Z,[-2,3.99], cmap='RdGy')
